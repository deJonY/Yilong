...
// ⬇️ YANGI: UI-da nomlarni avtomatik tarjima qilish
    ...
    <div className="space-y-10">
      ...
      <div className="space-y-4">
        ...
        {catsLoading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
            {categories.map((cat) => {
              // Alt atributi uchun i18n dan key orqali string topamiz (fallback bilan)
              const key = `catalog.sections.${sectionId}.categories.${cat.id}` as const;
              const tv = t(key);
              const label = (typeof tv === "string" && !tv.startsWith("catalog.")) ? tv : (cat.title || cat.id);
                ...
                >
                  {cat.image && (
                    ...
                    <img
                      loading="lazy"
                      src={cat.image}
                      alt={label /* alt string bo'lishi kerak */}
                      className="w-full h-40 object-cover rounded mb-3"
                    />
                  )}
                  <h3 className="text-lg font-semibold text-gray-800">
                    {/* Ko'rinadigan joyda esa lokallashgan ko'rinish */}
                    <LocalizedTitle text={label} />
                  </h3>
                </button>
              );
            })}
