// // Kanonik ID’lar, alias xaritalari va normalizatorlar
// // (Sidebar, Breadcrumbs va boshqalarda qayta ishlatiladi)
...
//   "kleevye-resheniya": { klei: "klei", skotch: "skotch" },
...
//   "frezy-i-gravery": { frezy: "frezy", gravery: "gravery" }
// };

// export function normalizeCatIdGuess(id: string): string[] {
...
//   variants.add(id.replace(/profil-dlya-lent/g, "alyuminievye-profili-dlya-svetodiodnyh-lent"));
...
// // I18n kalitlari uchun nomzodlar
// export function candidateSectionKeys(sectionId: string): string[] {
//   const sec = canonSectionId(sectionId);
//   return [`catalog.sections.${sec}.title`];
// }

// export function candidateCategoryKeys(sectionId: string, categoryId: string): string[] {
//   const sec = canonSectionId(sectionId);
//   const cat = canonCategoryId(sec, categoryId);
//   const set = new Set<string>();
//   set.add(`catalog.sections.${sec}.categories.${cat}`);
//   for (const v of normalizeCatIdGuess(cat)) {
//     set.add(`catalog.sections.${sec}.categories.${v}`);
//   }
